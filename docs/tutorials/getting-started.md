...
`explain` always evaluates in **read** mode — the default, and the safe default. The bundled `kubectl.rego` policy blocks every non-read verb (`delete`, `apply`, `scale`, …) when mode is `read`. `kubectl get`, `kubectl describe`, and `kubectl logs` would be allowed.
