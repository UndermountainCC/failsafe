...
failsafe intercepts each shell command via a `PreToolUse` hook, parses it into a structured fact (tool, verb, flags, cluster context), and evaluates it against a cascade of OPA Rego policies. It returns `allow`, `deny`, or `ask` before execution. There are no network calls and no telemetry, and every decision is logged locally.
