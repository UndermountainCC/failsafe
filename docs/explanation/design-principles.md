...
Three principles shape every decision in failsafe's implementation: it fails closed, it is local-first, and it keeps an honest record of every decision it makes. Each follows from a specific engineering constraint imposed by what failsafe is protecting.
